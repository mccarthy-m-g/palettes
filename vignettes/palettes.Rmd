...
The printing behaviour of colour vectors can be adjusted using a variety of global options. See `help("palettes-options")` for a list of the available options and their default values.

  - To change the symbol used for colour previews use `palettes.print_symbol`:
  ...
  - To print colour vectors compactly use `palettes.print_hex` and `palettes.print_width`:

    ```{r}
    options(palettes.print_hex = FALSE, palettes.print_width = 5)
    colour_palette
    ```
    
  - To separate colours on the same line use `palettes.print_sep`:

    ```{r}
    options(palettes.print_sep = " ")
    colour_vector
    ```
    
  - To show the index of the first colour on each line use `palettes.print_index`:

    ```{r}
    options(palettes.print_index = TRUE)
    colour_vector
    ```
