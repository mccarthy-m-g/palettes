...
To compliment `pal_colour()`, palettes provides `as_colour()` to coerce objects into colour vectors.
...
To interpolate or change the direction of colours in a plot, use the optional `n`, `direction`, or `interpolate` arguments.

```{r}
plot(colour_vector, n = 7, direction = -1, interpolate = "linear")
```

All plots are ggplot2 objects and can be customized using any of the standard ggplot2 methods. See the [customizing FAQ](https://ggplot2.tidyverse.org/articles/faq-customising.html) for some common examples.
